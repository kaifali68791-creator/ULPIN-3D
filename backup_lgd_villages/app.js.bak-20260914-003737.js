...
function renderParcelInfo() {
  ...
  const lgdExtra = isLGD ?
    '<div class="pd-item"><span>Data source</span><b style="color:var(--amber)">Real LGD (data.gov.in) + SAMPLE villages</b></div>' +
    '<div class="pd-item"><span>Village status</span><b style="color:var(--amber)">SAMPLE data — not official LGD village records</b></div>' +
...
/* ---- SAMPLE village dataset for LGD prototype mode -----------------------
   CLEARLY LABELLED SAMPLE/PROTOTYPE DATA — NOT official LGD village records.
   Deterministic: 3 villages per sub-district, codes derived from the
   official LGD sub-district code (never truncated or altered). Real LGD
   codes (state/district/sub-district) are preserved exactly as loaded. */
function initLgdSampleVillages() {
  if (typeof window.LGD_DATA === "undefined" || !window.LGD_DATA || !window.LGD_DATA.states) return false;
  if (window.LGD_SAMPLE_VILLAGES_READY) return true;
  window.LGD_DATA.states.forEach((s) => {
    (s.districts || []).forEach((d) => {
      (d.tehsils || []).forEach((t) => {
        t.sample = true; /* mark sub-district village data as SAMPLE */
        t.villages = [1, 2, 3].map((n) => ({
          code: String(t.code) + "0" + n,           /* deterministic, derived, never alters LGD codes */
  ...
  window.LGD_SAMPLE_VILLAGES_READY = true;
  return true;
}
  ...
  const setToggleUI = () => {
    ...
    if (lgdNotice) {
      ...
      lgdNotice.innerHTML = isLGD ?
        '<strong style="color:var(--amber)">PROTOTYPE ULPIN - NOT AN OFFICIAL GOVERNMENT ID</strong><br>' +
        'State / District / Sub-District use real LGD data from data.gov.in (Ministry of Panchayati Raj). ' +
        'Villages are <strong>SAMPLE / PROTOTYPE data</strong> for demo only - the official LGD village dataset is not loaded. ' +
        'The generated RULPIN is a reference identifier, not an official government-issued ULPIN.' :
        "";
    }
  };
